...
function FileView({ file }: { file: File }) {
    ...
    const [getDirectory, open, setOpen, addSelected] = useNoterState(
        (state) => [
            state.getDirectory,
            state.open,
            state.setOpen,
    ...
    async function handleOpenInNewTab() {
        await addSelected(file);
    }

    async function handleRename() {
        await putFile(file.id, name, "Note");
        await getDirectory();
    }
                    ...
                    <div className="w-full h-full">
                        {dir.entries.map((entry) => {
                            if (entry.Directory) {
                                ...
                                return (
                                    <DirectoryView key={dir.id.id} dir={dir} />
                                );
                            } else if (entry.File) {
                                const file = entry.File;
                                return (
                                    <FileView key={file.id.id} file={file} />
                                );
                            }
                        })}
                    </div>
